...
interface ThemeContextType {
  ...
  setUserObj: Dispatch<SetStateAction<UserRoot>>;
  getSubtotal: () => number;
  getItemCount: () => number;
  clearCart: () => void;
}
...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [userObj, setUserObj] = useState<UserRoot>({} as UserRoot);
  ...
  const Authentication = async () => {
    ...
    const response:Promise<UserRoot> | undefined=await request?.data
    const responseStatus: number | undefined = await request?.status;
    console.log("Authentication Response:", request);
    // if (request?.data) {
    //   
    ...
    if (responseStatus===undefined){
      console.warn("Sorry an unexpected error occured login or signup ")
       setAuthLoading(false)
        setLoggedIn(false)
      ...
      console.log("please Login Your Account")
       setAuthLoading(false)
      setLoggedIn(false)
    }else if (responseStatus===200 && response){
      setLoggedIn(true)
      setAuthLoading(false)
      setUserObj(request?.data)
    }
...
export function CartProvider({ children }: { children: ReactNode }) {
  const product: Product2 = {
    ...
    sku: "TN-X200-BLK",
    fullDescription:
      "The Smartphone X200 combines style, speed, and innovation. With a high-resolution display, advanced camera system, and long-lasting battery, it's perfect for tech enthusiasts.",
    features: [
    ...
    warrantyInformation:
      "2-year limited warranty covering manufacturing defects.",
    shippingInformation:
      "Ships within 3–5 business days. Free shipping on orders over $500.",
    ...
    thumbnail:
      "https://res.cloudinary.com/dt6naawfk/image/upload/v1754584716/Post-images/tmp-1-1754584715767_nevtgm.jpg",
  };
  const product2: Product2 = {
    _id: 2025,
    title: 'UltraView 27" 4K Monitor',
    description:
      "A professional-grade 4K UHD monitor with vibrant color accuracy.",
    ...
    sku: "PP-UV27-4K",
    fullDescription:
      'Designed for creatives and professionals, the UltraView 27" delivers stunning visuals with 99% AdobeRGB coverage and HDR10 support.',
    features: [
      "27-inch IPS panel",
    ...
    thumbnail:
      "https://res.cloudinary.com/dt6naawfk/image/upload/v1757257001/Post-images/tmp-1-1757257000721_zr4klv.jpg",
    image: [
      {
  ...
  const [items, setItems] = useState<CartItem2[]>([
...
export function CheckoutProvider({ children }: { children: ReactNode }) {
  ...
  const [customerDetails, setCustomerDetailsState] =
    useState<customerDetails2 | null>(null);
  const [selectedShipping, setSelectedShippingState] =
