...
export async function generateMetadata({ params: { orderId } }: SingleProp) {
  const wikiData: Promise<any> = await getSingleOrder(orderId);
  const data: any | undefined = (await wikiData)?.order;
  console.log(wikiData);
  if (data === undefined) {
    return {
      title: "This Order is missing",
      description: "No results for this product found",
    };
  }
  return {
    title: "Admin |" + " " + data.name + "'s Order",
...
export default async function SingleOrder({ params: { orderId } }: SingleProp) {
  const wikiData: Promise<any> = await getSingleOrder(orderId);
  const data: adminOrder | undefined = (await wikiData)?.order;
  console.log(wikiData);
  return (
    <div className="grow p-5 lg:rounded-lg lg:bg-white lg:p-10 lg:shadow-sm lg:ring-1 lg:ring-zinc-950/5 dark:lg:bg-zinc-900 dark:lg:ring-white/10">
          ...
          <>
            <div className="mt-4 lg:mt-8">
              <div className="flex items-center gap-4">
                <h1 className="text-2xl/8 text-zinc-950 sm:text-xl/8 dark:text-white">
                  Order #{data._id}
                </h1>
              ...
              <div className="isolate mt-2.5 flex flex-wrap justify-between gap-x-6 gap-y-4">
                ...
                <div className="flex gap-4">
                  <button
                    type="button"
                    className="relative isolate inline-flex items-baseline justify-center gap-x-2 rounded-lg border text-base/6 px-3 py-2 sm:px-2.5 sm:py-1 sm:text-sm/6 focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-500 disabled:opacity-50 border-zinc-950/10 text-zinc-950 active:bg-zinc-950/5 hover:bg-zinc-950/5 dark:border-white/15 dark:text-white dark:bg-transparent dark:active:bg-white/5 dark:hover:bg-white/5 cursor-default"
                    data-headlessui-state=""
                  >
                    <span
            ...
            <div className="mt-12">
              <h2 className="text-base/7 text-zinc-950 sm:text-sm/6 dark:text-white">
                Summary
              </h2>
              <hr
                role="presentation"
              ...
              <div className="space-y-6">
                <div className="flex justify-center">
                  <div className="relative">
                    <div className="relative bg-gradient-to-br from-blue-50 to-blue-100 rounded-full p-6">
                      <PackageOpen
